...
namespace ldmx {
    ...
    LcioPersistencyManager::LcioPersistencyManager() :
        G4PersistencyManager(G4PersistencyCenter::GetPersistencyCenter(), "LcioPersistencyManager") {
            ...
            writer_ = nullptr;
            builder_ = new SimParticleBuilder(); // FIXME: Probably shouldn't set this here!
        }
    ...
    G4bool LcioPersistencyManager::Store(const G4Event* anEvent) {
        if (!anEvent->IsAborted()) {
            ...
            builder_->setCurrentEvent(anEvent);

            // Write SimParticles to LCIO event (allowed to be empty).
            if (m_verbose > 1) {
                std::cout << "LcioPersistencyManager: Storing MC particles in event "
                    << anEvent->GetEventID() << std::endl;
            }
            builder_->buildSimParticles(lcioEvent);
            ...
            // Print final number of objects in collections.
            if (m_verbose > 1) {
    ...
    void LcioPersistencyManager::writeHitsCollections(const G4Event* g4Event, IMPL::LCEventImpl* lcioEvent) {
        ...
        if (hce) {
            ...
            for (int iColl = 0; iColl < nColl; iColl++) {
                ...
                std::cout << "Collection Name: " << collName << std::endl;
                if(collName == "TaggerSimHits") continue;
                if(collName == "RecoilSimHits") continue;
                if(collName == "TargetSimHits") continue;
                if(collName == "TriggerPadTaggerSimHits") continue;
                if(collName == "TriggerPadUpSimHits") continue;
                if(collName == "TriggerPadDownSimHits") continue;
    ...
    IMPL::LCCollectionVec* LcioPersistencyManager::writeTrackerHitsCollection(G4VHitsCollection* hc) {
        ...
        for (int i = 0; i < nhits; i++) {
            ...
            // get the SimParticle for the hit
            if (m_verbose > 3) {
                std::cout << "LcioPersistencyManager: Looking for track ID " << trackerHit->getTrackID() << std::endl;
            }
            IMPL::MCParticleImpl* mcp = builder_->findLcioParticle(trackerHit->getTrackID());
            if (!mcp) {
                std::cerr << "LcioPersistencyManager: No SimParticle found for trackID " << trackerHit->getTrackID()
                    << " from sim tracker hit" << std::endl;
                G4Exception("LcioPersistencyManager::writeHitsCollections", "", FatalException,
                        "SimParticle for track ID is missing.");
            } else {
    ...
    IMPL::LCCollectionVec* LcioPersistencyManager::writeCalorimeterHitsCollection(G4VHitsCollection* hc) {
        ...
        for (int i = 0; i < nhits; i++) {
            ...
            for (auto contrib : contribs) {
                ...
                G4VTrajectory* traj = builder_->findTrajectory(trackID);
                if (!traj) {
                ...
                auto mcp = builder_->findLcioParticle(traj->GetTrackID());
                if (!mcp) {
